...
import { changeCard } from '../../Server/query';
import { Formik } from "formik";
import * as yup from "yup";
import { useStyles } from './style';

export default function DialogForChangeCard(props) {
  ...
  return (
    <Dialog open={open} onClose={handleClose} className={root}>
      <Formik
        initialValues={{ name: card.name, phone: card.number }}
        validateOnChange
        validationSchema={validationSchema}
        onSubmit={(values) => {
          changeCard(card.id,{ name: values.name, number: values.phone, userId: card.userId })
          handleClose()
        }}
      >
        {({
          ...
          <form onSubmit={handleSubmit} className={form}>
            ...
            <DialogActions>
              ...
              <Button type="submit"
                disabled={!isValid && !dirty}>Change</Button>
            </DialogActions>
          </form>
        )}
      </Formik>
